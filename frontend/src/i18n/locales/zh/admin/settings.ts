export default {
    settings: {
      ...
      payment: {
        ...
        validationEasyPayCustomMethodUpstreamTypeInvalid: '易支付上游 type 必须由点号分隔的小写字母、数字、下划线或短横线组成',
        ...
        field_queryApiPath: '查单接口路径',
        field_querySignType: '查单签名类型',
        field_queryPrivateKey: '查单私钥',
        field_queryPublicKey: '查单返回公钥',
        field_easypayQueryApiPathHint: '可选。填写后会优先使用新版查单；默认 /api/pay/query，也可填写其它易支付服务商的完整查单 URL。',
        field_easypayQuerySignTypeHint: '可选。填写后会优先使用新版 /api/pay/query 查单；不填则使用兼容易支付 legacy 查单。',
        field_easypayQueryPrivateKeyHint: '新版 RSA/RSA2 查单必填。支持 PEM 或 Base64 DER；不同易支付服务商可使用自己的 RSA 私钥。',
        field_easypayQueryPublicKeyHint: '可选。填写服务商平台公钥后会校验查单返回签名；不填则仅依赖 HTTPS 与请求签名。',
        field_cid: '支付渠道 ID',
