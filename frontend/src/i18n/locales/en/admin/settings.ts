export default {
    settings: {
      ...
      payment: {
        ...
        validationEasyPayCustomMethodUpstreamTypeInvalid: 'EasyPay upstream types must be dot-separated lowercase letters, digits, underscores, or hyphens',
        ...
        field_queryApiPath: 'Query API path',
        field_querySignType: 'Query sign type',
        field_queryPrivateKey: 'Query private key',
        field_queryPublicKey: 'Query response public key',
        field_easypayQueryApiPathHint: 'Optional. When set, the newer query is tried first. Default is /api/pay/query; a full query URL for another EasyPay-compatible gateway is also accepted.',
        field_easypayQuerySignTypeHint: 'Optional. When set, the provider uses the newer /api/pay/query order query first; otherwise it keeps the legacy EasyPay query.',
        field_easypayQueryPrivateKeyHint: 'Required for RSA/RSA2 query signing. PEM or Base64 DER are supported; this is provider-specific, not tied to one EasyPay domain.',
        field_easypayQueryPublicKeyHint: 'Optional. Set the gateway platform public key to verify signed query responses; leave blank to rely on HTTPS and request signing only.',
